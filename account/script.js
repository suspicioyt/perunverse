const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzGu4orOZhX9Cf0G1tR38dMGiTtYdLIejJSjPC6FOlkBnl2KCnP9ub31-OsamR87lJU/exec';
        ...
        function validateEmail(email) {
            if (!email) return true; // Email jest opcjonalny
            const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        ...
        function selectGender() {
            ...
            document.getElementById('genderDisplay').value = selectedGender;
        ...
        function register() {
            ...
            const params = `action=register&username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&birthYear=${birthYear}&gender=${encodeURIComponent(gender)}${email ? `&email=${encodeURIComponent(email)}` : ''}`;
